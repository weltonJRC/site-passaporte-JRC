...
2. Configure `SMTP_HOST`, `SMTP_PORT`, `SMTP_USER`, `SMTP_PASSWORD` e `SMTP_FROM` para o botão **Enviar todos por e-mail** e para a recuperação de acesso. Sem SMTP, o envio em massa por e-mail retorna erro e a recuperação fica indisponível em produção.
...
- **Enviar todos por e-mail** processa convites e participantes em lotes de 50. Convites pendentes recebem um novo link de ativação; participantes com convite usado recebem somente o link de login. Destinatários sem e-mail são marcados como ignorados. Uma falha individual aparece no resultado do lote.
- **Preparar WhatsApp de todos** monta mensagens e abre links de conversa individualmente. O botão não envia mensagens automaticamente: o WhatsApp usado hoje é o link `api.whatsapp.com/send`, que exige a confirmação de envio na conversa.
